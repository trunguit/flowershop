...
class MailService
{
    ...
    public function sendOrderInfo($data)
    {
        $mail = json_decode(SettingModel::where('key_value', 'setting-email')->first()->value, true);
        if (empty($mail))
            return false;
            ...
            Mail::send([], [], function ($message) use ($mail, $data) {
                ...
                $message->subject('Thông tin đặt hàng từ ' . $data['name']);
                $content = sprintf('
                <p>Name: %s</p>
                <p>Email: %s</p>
                <p>Phone: %s</p>
                <p>Message: %s</p>
                ', $data['name'], $data['email'], $data['phone'], $data['note']);
                $message->setBody($content, 'text/html');
            });
            return true;
        }
    }
    public function sendConfirmOder($data)
    {
        $mail = json_decode(SettingModel::where('key_value', 'setting-email')->first()->value, true);
        if (empty($mail))
            return false;
            ...
            Mail::send([], [], function ($message) use ($mail, $data) {
                ...
                $content = sprintf('
                <p>Xin chào, %s</p>
                <p>Hoa Shop đã nhận được thông tin đặt hàng từ bạn. </p>
                <p>Mã đơn hàng của bạn là %s sẽ gửi hoa cho bạn trong thời gian sớm nhất</p>
                <p>Cảm ơn!</p>
                ',$data['idCart'] ,$data['name']);
                $message->setBody($content, 'text/html');
            });
            return true;
        }
